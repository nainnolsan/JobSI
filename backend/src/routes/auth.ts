...
router.get('/cover-letters', authenticateJWT, async (req: Request, res: Response) => {
  const userId = req.user?.userId;
  try {
    const result = await pool.query('SELECT id, title, company, status, created_at, updated_at FROM cover_letters WHERE user_id = $1 ORDER BY created_at DESC', [userId]);
    res.json(result.rows);
  } catch (err) {
...
router.post('/cover-letters', authenticateJWT, async (req: Request, res: Response) => {
  const userId = req.user?.userId;
  const { title, company, raw_job_text, parsed, config } = req.body;
  try {
    const result = await pool.query(
      `INSERT INTO cover_letters (user_id, title, company, raw_job_text, parsed, config, status) VALUES ($1,$2,$3,$4,$5,$6,'draft') RETURNING *`,
...
router.get('/cover-letters/:id', authenticateJWT, async (req: Request, res: Response) => {
  const userId = req.user?.userId;
  const id = req.params.id;
  try {
    const result = await pool.query('SELECT * FROM cover_letters WHERE id=$1 AND user_id=$2', [id, userId]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Cover letter no encontrada' });
    res.json(result.rows[0]);
  } catch (err) {
...
router.put('/cover-letters/:id', authenticateJWT, async (req: Request, res: Response) => {
  const userId = req.user?.userId;
  const id = req.params.id;
  const { title, company, raw_job_text, parsed, config, generated_draft, status } = req.body;
  try {
    const result = await pool.query(
      `UPDATE cover_letters SET title=$1, company=$2, raw_job_text=$3, parsed=$4, config=$5, generated_draft=$6, status=$7, updated_at = CURRENT_TIMESTAMP WHERE id=$8 AND user_id=$9 RETURNING *`,
      [title || null, company || null, raw_job_text || null, parsed || null, config || null, generated_draft || null, status || 'draft', id, userId]
    );
    if (result.rows.length === 0) return res.status(404).json({ error: 'Cover letter no encontrada' });
    res.json(result.rows[0]);
  } catch (err) {
...
router.delete('/cover-letters/:id', authenticateJWT, async (req: Request, res: Response) => {
  const userId = req.user?.userId;
  const id = req.params.id;
  try {
    const result = await pool.query('DELETE FROM cover_letters WHERE id=$1 AND user_id=$2 RETURNING *', [id, userId]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Cover letter no encontrada' });
...
router.post('/cover-letters/parse', authenticateJWT, async (req: Request, res: Response) => {
  const { raw_job_text } = req.body;
  if (!raw_job_text || typeof raw_job_text !== 'string') return res.status(400).json({ error: 'raw_job_text requerido' });
  ...
  try {
    ...
    const parsed: any = { title, company, responsibilities: [], requirements: [], keywords: [] };
...
router.post('/cover-letters/generate', authenticateJWT, async (req: Request, res: Response) => {
  const userId = req.user?.userId;
  const { parsed, config, user_profile, cover_letter_id, variants = 1 } = req.body;
  if (!parsed || !config) return res.status(400).json({ error: 'parsed y config requeridos' });

  if (!process.env.OPENAI_API_KEY) return res.status(500).json({ error: 'OpenAI API key no configurada' });
  ...
  try {
    ...
    if (cover_letter_id && content.length > 0) {
      await pool.query('UPDATE cover_letters SET generated_draft=$1, variants=$2, status=$3, updated_at=CURRENT_TIMESTAMP WHERE id=$4 AND user_id=$5', [content[0], JSON.stringify(content), 'generated', cover_letter_id, userId]);
    }
